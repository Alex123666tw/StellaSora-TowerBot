...
echo =====================================================
echo   Stella Sora Safe Single Round
echo =====================================================
echo [INFO] This runs max_runs=1 with the external watchdog.
echo [INFO] Use windowed or borderless-windowed mode.
echo [INFO] Game and bot should use the same privilege level.
echo.

"%PYTHON_EXE%" diagnostics\safe_single_round_test.py --input-mode foreground --stuck-timeout 30 --sample-interval 1
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if %EXIT_CODE% neq 0 (
    echo [FAIL] safe_single_round_test.py exited with code %EXIT_CODE%.
) else (
    echo [PASS] safe_single_round_test.py completed.
)
echo =====================================================
pause
exit /b %EXIT_CODE%
