...
"%PYTHON_EXE%" diagnostics\active_window_probe.py --no-ocr
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if %EXIT_CODE% neq 0 (
    echo [FAIL] active_window_probe.py exited with code %EXIT_CODE%.
) else (
    echo [PASS] active_window_probe.py completed.
)
echo =====================================================
pause
exit /b %EXIT_CODE%
