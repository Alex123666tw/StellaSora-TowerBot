...
"%PYTHON_EXE%" diagnostics\auto_execute_bot.py
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if %EXIT_CODE% neq 0 (
    echo [FAIL] auto_execute_bot.py exited with code %EXIT_CODE%.
) else (
    echo [PASS] auto_execute_bot.py completed.
)
echo =====================================================
pause
exit /b %EXIT_CODE%
