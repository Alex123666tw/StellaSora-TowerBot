...
**星塔旅人自動探索輔助**是一款執行於 Windows 的桌面輔助程式,專為遊戲《星塔旅人》設計。它的運作流程是:
...
- **可設定的決策引擎**:潛能選卡支援「保底優先」、「必選清單」與重抽降級備選;隨機事件依權重挑選較佳選項;商店依折扣與音符缺口決定採購與重置。
- **音符 / 協奏追蹤**:以白色內符號加色相辨識讀取秘紋啟動所需音符,輔助採購與決策。
- **攻略圖解析**:可從攻略截圖以 OCR 解析必拿 / 備選技能,並更新決策設定。
...
- 建議以 **1920×1080 視窗模式(非全螢幕)** 執行遊戲
- 建議配備 NVIDIA 顯卡(EasyOCR 可使用 CUDA 加速,非必要)
- 需要 **系統管理員權限** 執行(程式啟動時會檢查;遊戲與本程式建議使用相同權限等級)
...
1. 以 **1920×1080 視窗模式(非全螢幕)** 開啟《星塔旅人》。
...
4. 在 GUI 中可上傳攻略圖進行解析、調整參數,並透過即時日誌面板觀察當前執行狀態與動作。
...
> 若日後想改回 MIT 等寬鬆授權,須先移除這些 GPL 相依(注意只換 PySide6 不夠,Fluent-Widgets 元件庫本身也是 GPL,須一併換成非 copyleft 的 UI),或向上游購買商用授權。
